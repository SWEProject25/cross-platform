...
final profilePostsProvider = NotifierProvider.family<
    ProfilePostsPaginationNotifier,
    PaginationState<TweetModel>,
    String>(
  ProfilePostsPaginationNotifier.new,
);

class ProfilePostsPaginationNotifier
    extends PaginationNotifier<TweetModel> {

  final String userId;
  ProfilePostsPaginationNotifier(this.userId);
  ...
  Future<(List<TweetModel>, bool)> fetchPage(int page) async {
    final raw = await _api.getProfilePosts(userId, page, pageSize);
