...
void main() {
  ...
  group('MentionNotificationsViewmodel Tests', () {
    ...
    setUp(() {
      ...
      // Default stubs
      when(() => mockRepository.fetchMentionNotifications(any(), any()))
    ...
    group('build()', () {
      test('should initialize with empty PaginationState', () {
        final state = container.read(mentionNotificationsViewModelProvider);
      ...
      test('should call loadInitial on build', () async {
        when(() => mockRepository.fetchMentionNotifications(1, 20))
            .thenAnswer((_) async => ([mockNotification1], true));

        // Trigger build by reading the provider
        container.read(mentionNotificationsViewModelProvider);

        // Wait for microtask to complete
        await Future.microtask(() {});
        await Future.delayed(Duration.zero);

        // Verify fetchMentionNotifications was called
        verify(() => mockRepository.fetchMentionNotifications(1, 20)).called(1);
      });
    ...
    group('fetchPage()', () {
      test('should fetch notifications for given page', () async {
        ...
        final viewmodel =
            container.read(mentionNotificationsViewModelProvider.notifier);
      ...
      test('should fetch multiple pages correctly', () async {
        when(() => mockRepository.fetchMentionNotifications(1, 20))
            .thenAnswer((_) async => ([mockNotification1], true));
        when(() => mockRepository.fetchMentionNotifications(2, 20))
        ...
        verify(() => mockRepository.fetchMentionNotifications(1, 20)).called(1);
        verify(() => mockRepository.fetchMentionNotifications(2, 20)).called(1);
      });
    });
    ...
    group('markAllAsRead()', () {
      test('should mark all notifications as read', () async {
        when(() => mockRepository.fetchMentionNotifications(1, 20)).thenAnswer(
          (_) async => ([
            mockNotification1,
            mockNotification2,
          ], false),
      ...
      test('should update notification count after marking all as read', () {
        ...
        final viewmodel =
            container.listen(mentionNotificationsViewModelProvider.notifier, (_, __) {}).read();
    ...
    group('PaginationNotifier inherited methods', () {
      test('loadInitial should set loading state', () async {
        ...
        // await Future.delayed(Duration.zero);
        var state = container.read(mentionNotificationsViewModelProvider);
        expect(state.isLoading, isTrue);

        // Wait for completion
        await loadFuture;

        state = container.read(mentionNotificationsViewModelProvider);
        expect(state.isLoading, isFalse);
      ...
      test('loadMore should append new items', () async {
        ...
        when(() => mockRepository.fetchMentionNotifications(2, 20)).thenAnswer(
      ...
      test('should not load more when hasMore is false', () async {
        ...
        verify(() => mockRepository.fetchMentionNotifications(1, 20)).called(1);
        verifyNever(() => mockRepository.fetchMentionNotifications(2, 20));
      });
