...
void main() {
  ...
  group('UsersToFollowViewmodel - build (Initial State)', () {
    test('should load users to follow successfully on build', () async {
      ...
      expect(users[0].profile?.name, 'John Doe');
      expect(users[1].profile?.name, 'Jane Smith');
      expect(users[2].profile?.name, 'Bob Wilson');
  ...
  group('UsersToFollowViewmodel - Edge Cases', () {
    test('should handle user with null profile', () async {
      final mockUsers = [
        UserToFollowDto(
          id: 1,
          username: 'johndoe',
      ...
      expect(users.length, 1);
      expect(users[0].profile, null);
    });

    test('should handle user with null profile fields', () async {
      final mockUsers = [
        UserToFollowDto(
          id: 1,
          username: 'johndoe',
      ...
      expect(users.length, 1);
      expect(users[0].profile?.profileImageUrl, null);
      expect(users[0].profile?.bio, null);
    });
