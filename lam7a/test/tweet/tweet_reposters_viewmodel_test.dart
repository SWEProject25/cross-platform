...
void main() {
  ...
  group('TweetRepostersViewModel Tests', () {
    test('build calls getReposters and returns users', () async {
      final users = [
        UserModel(id: 1, username: 'user1', email: 'u1@test.com'),
        UserModel(id: 2, username: 'user2', email: 'u2@test.com'),
      ];

      when(() => mockService.getReposters('123')).thenAnswer((_) async => users);

      final result = await container.read(tweetRepostersViewModelProvider('123').future);

      expect(result, users);
      verify(() => mockService.getReposters('123')).called(1);
    });

    test('build handles empty list', () async {
      when(() => mockService.getReposters('123')).thenAnswer((_) async => []);
    ...
    test('build handles error', () async {
      when(() => mockService.getReposters('123')).thenThrow(Exception('Network error'));

      expect(
        () => container.read(tweetRepostersViewModelProvider('123').future),
        throwsA(isA<Object>()),
      );
    });
  });
