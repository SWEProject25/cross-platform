...
void main() {
  ...
  setUp(() {
    ...
    when(() => mockSocket.incomingMessages).thenAnswer((_) => const Stream.empty());
    when(() => mockSocket.incomingMessagesNotifications).thenAnswer((_) => const Stream.empty());
  });
  ...
  group('ConversationsNotifier (Pagination) tests', () {
    test('provider build triggers loadInitial and populates items', () async {
      ...
      final authState = AuthState(isAuthenticated: true, user: UserModel(id: 999, email: 'a@b.com'));
    ...
    test('loadInitial handles repository errors', () async {
      ...
      final authState = AuthState(isAuthenticated: true, user: UserModel(id: 1, email: 'x'));
    ...
    test('loadMore merges new items and updates hasMore correctly', () async {
      ...
      final pageItems = List.generate(20, (i) => Conversation(id: i + 1, userId: 0, name: 'n', username: 'u', avatarUrl: null));
      when(() => mockRepo.fetchConversations()).thenAnswer((_) async => pageItems);

      final authState = AuthState(isAuthenticated: true, user: UserModel(id: 2, email: 'a@b'));
      container = ProviderContainer(
        overrides: [
          conversationsRepositoryProvider.overrideWithValue(mockRepo),
          messagesSocketServiceProvider.overrideWithValue(mockSocket),
      ...
      final notifier = container.read(conversationsProvider.notifier)..loadInitial();

      // Now stub _fetchPage called by loadMore to return fewer items (e.g., 5) -> hasMore false
      when(() => mockRepo.fetchConversations()).thenAnswer((_) async => List.generate(5, (i) => Conversation(id: 100 + i, userId: 0, name: 'n', username: 'u', avatarUrl: null)));
    ...
    test('loadMore returns early when already loading or no more', () async {
      when(() => mockRepo.fetchConversations()).thenAnswer((_) async => <Conversation>[]);
      final authState = AuthState(isAuthenticated: true, user: UserModel(id: 3, email: 'x'));
  ...
  group('onNewMessageReceived behavior', () {
    test('message for existing conversation from same user => early return (no change)', () async {
      final item = conv(id: 10, lastMessageTime: DateTime(2024, 1, 1), lastMessage: 'old');
      when(() => mockRepo.fetchConversations()).thenAnswer((_) async => [item]);
      final authState = AuthState(isAuthenticated: true, user: UserModel(id: 42, email: 'x'));
      ...
      final message = chatMsg(id: 1, senderId: 42, conversationId: 10, time: DateTime.now(), text: 'new-text');
    ...
    test('message for existing conversation from other user updates conversation and sorts', () async {
      final convA = conv(id: 1, lastMessageTime: DateTime(2023, 1, 1), lastMessage: 'a');
      final convB = conv(id: 2, lastMessageTime: DateTime(2022, 1, 1), lastMessage: 'b');

      when(() => mockRepo.fetchConversations()).thenAnswer((_) async => [convA, convB]);
      final authState = AuthState(isAuthenticated: true, user: UserModel(id: 999, email: 'a@b'));
      ...
      final message = chatMsg(id: 10, senderId: 5, conversationId: 2, time: messageTime, text: 'hello!');
    ...
    test('message for unknown conversation fetches contact and merges new conversation', () async {
      // initial empty list
      when(() => mockRepo.fetchConversations()).thenAnswer((_) async => <Conversation>[]);
      when(() => mockRepo.getContactByUserId(77)).thenAnswer((_) async =>
          Contact(id: 77, name: 'New Guy', handle: 'newguy', avatarUrl: 'a.jpg'));

      final authState = AuthState(isAuthenticated: true, user: UserModel(id: 1000, email: 'me@x'));
      ...
      final message = chatMsg(id: 20, senderId: 77, conversationId: 500, time: DateTime(2024, 6, 1), text: 'hey there');
